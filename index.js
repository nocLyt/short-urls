...
var shortenURLByLongURL =require("./Utils/urls").shortenURLByLongURL;
var redirectByShortURL = require('./Utils/decodeURL').redirectByShortURL;
...
server.get(/shorten\/?(.*)/, shortenURLByLongURL);
// server.get(/shorten\/?(.*)/, showOriginURL);

// // 2. Decode shortURL to rederict longURL
server.get(/s\/(.*)/, redirectByShortURL);

server.get('/1', function (req, res, next) {
    res.redirect('http://www.mi.com/us/', next);
});
